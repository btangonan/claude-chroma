...
# Version 3.1 - Fixed critical bugs:
# - Removed invalid settings.local.json creation (Claude schema doesn't support "instructions" field)
# - Fixed command suggestions (removed --mcp-config which doesn't work with settings format)
# - Made paths portable (uses intelligent $HOME-based detection)
# - Added migration for existing v3.0 installations
...
if [ -z "$PROJECT_NAME" ]; then
    ...
    if [ -z "${2:-}" ]; then
        # Intelligent project path detection
        if [ -d "$HOME/projects" ]; then
            DEFAULT_PATH="$HOME/projects"
        elif [ -d "$HOME/Documents/projects" ]; then
            DEFAULT_PATH="$HOME/Documents/projects"
        elif [ -d "$HOME/Desktop/projects" ]; then
            DEFAULT_PATH="$HOME/Desktop/projects"
        else
            DEFAULT_PATH="$HOME"
            print_info "Using home directory as default. Consider creating ~/projects/"
        fi
        if [ "$YES" = "1" ]; then
...
# Step 3: Create .mcp.json for project-specific MCP server
...
if [ -f ".mcp.json" ]; then
    ...
    if grep -q '"chroma"' .mcp.json 2>/dev/null; then
        echo -e "${YELLOW}ChromaDB MCP server already configured${NC}"
        confirm "Overwrite existing ChromaDB config?"
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            print_info "Keeping existing ChromaDB configuration"
            SKIP_MCP_CONFIG=true
        else
            BACKUP_NAME=".mcp.json.backup.$(safe_timestamp)"
            print_info "Backing up existing .mcp.json to $BACKUP_NAME"
            cp .mcp.json "$BACKUP_NAME"
            SKIP_MCP_CONFIG=false
        fi
    else
        # Has .mcp.json but no chroma - needs merge
        echo -e "${YELLOW}Existing .mcp.json found without ChromaDB${NC}"
        print_info "Merging ChromaDB into existing MCP configuration"

        # Create backup first
        BACKUP_NAME=".mcp.json.backup.$(safe_timestamp)"
        cp .mcp.json "$BACKUP_NAME"
        CLEANUP_FILES+=(".mcp.json")
        ...
        if [ "$HAS_JQ" = "true" ]; then
            ...
            ' .mcp.json > "$TEMP_FILE" && mv "$TEMP_FILE" .mcp.json
            print_status "Merged ChromaDB into .mcp.json using jq"
...
"
            print_status "Merged ChromaDB into .mcp.json using Python"
        else
            print_error "Neither jq nor Python available for merge. Creating new file."
            SKIP_MCP_CONFIG=false
        fi
        SKIP_MCP_CONFIG=true
    fi
...
if [ "$SKIP_MCP_CONFIG" != "true" ]; then
    # Create .mcp.json with ChromaDB configuration
    cat > .mcp.json <<EOF
{
  "mcpServers": {
    "chroma": {
...
EOF
    print_status "Created .mcp.json with ChromaDB configuration"
fi
...
# Migration from v3.0 - Remove invalid settings.local.json if it exists
if [ -f ".claude/settings.local.json" ] && grep -q '"instructions"' .claude/settings.local.json 2>/dev/null; then
    print_info "Found invalid settings.local.json from v3.0, removing..."
    rm .claude/settings.local.json
    print_status "Cleaned up invalid configuration"
    # Remove empty .claude directory if no other files
    if [ -d ".claude" ] && [ -z "$(ls -A .claude 2>/dev/null)" ]; then
        rmdir .claude
        print_status "Removed empty .claude directory"
    fi
fi
...
# Step 5: Create simple initialization instructions
...
# Step 6: Create project launcher script
...
function claude-chroma --description "Start Claude with auto-detected ChromaDB config"
    ...
    if test -n "$config_file"
        ...
        # CD to project directory so Claude auto-loads .mcp.json
        cd "$project_dir"
...
echo ""
print_status "ChromaDB MCP server configured in .mcp.json"
print_status "Instructions configured in CLAUDE.md"
print_status "No Python dependencies required"
print_status "Claude will auto-detect .mcp.json when started in project"
echo ""
...
if [[ $SHELL_FUNCTION_REPLY =~ ^[Yy]$ ]] && [ -f "$SHELL_CONFIG" ] && ! grep -q "claude-chroma()\|function claude-chroma" "$SHELL_CONFIG" 2>/dev/null; then
    ...
    echo "     OR: claude chat  # Claude auto-detects .mcp.json"
fi
