#!/usr/bin/env bash
# ChromaDB + Claude MCP Setup (v4)
# Goal: clean, idempotent, CI-friendly, and safe to rerun
# Usage:
#   CHROMA_SETUP_YES=1 ./chromadb_setup_v4.sh "<ProjectName>" "<ParentPath>"
...
backup_and_merge_settings(){
  local path="$1"
  local tmp="${path}.tmp.$$"
  local bak="${path}.backup.$(date +%Y%m%d_%H%M%S)"
  if [[ -f "$path" ]]; then
    cp "$path" "$bak"
    info "Backup created: $bak"
    if [[ "$JQ_FOUND" == "1" ]]; then
      # Merge with jq: add/replace mcpServers.chroma and ensure instructions union
...
PY
      # Feed CONFIG_JSON via stdin
    else
      warn "Neither jq nor python3 available. Replacing settings file."
      printf "%s\n" "$CONFIG_JSON" > "$tmp"
    fi
    # If Python path used, we must re-run feeding CONFIG_JSON through stdin
    if [[ "$PYTHON_FOUND" == "1" && "$JQ_FOUND" != "1" ]]; then
      # Re-run python merge with CONFIG_JSON piped
